...
def predictm():
    try:
        ...
        # Save to database (adjusting for 13 features)
        cursor = mysql.connection.cursor()
        sql = """
            INSERT INTO `heart_disease_test`
            (`age`, `sex`, `cp`, `trestbps`, `chol`, `fbs`, `restecg`, `thalach`, `exang`, `oldpeak`, `slope`, `ca`, `thal`, `name`, `mo`, `emailid`, `timestamp`)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
        """
        values = tuple(features + [patient_details['name'], patient_details['mo'], patient_details['emailid'], patient_details['timestamp']])
